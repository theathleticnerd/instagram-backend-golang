...
func GetPostOFUser(w http.ResponseWriter, r *http.Request) {
	...
	IntUserID, _ := strconv.ParseInt(UserID, 0, 64)
	w.Header().Set("content-type", "application/json")
	param1 := r.URL.Query().Get("PageNumber")
	IntPageNeed, _ := strconv.ParseInt(param1, 0, 64)
	if IntPageNeed == 0 {
		IntPageNeed = 1
	}
	perPage := 5
	fmt.Println(IntPageNeed)
	if r.Method == "GET" {
		client, err := mongo.NewClient(options.Client().ApplyURI("mongodb+srv://job:y0SFqFVTmz9uDBEY@cluster0.xpnzm.mongodb.net/instaDB?retryWrites=true&w=majority"))
		ctx, _ := context.WithTimeout(context.Background(), 15*time.Second)
		err = client.Connect(ctx)
		...
		collection := client.Database("users").Collection("usersinstaclone")
		collection.FindOne(ctx, bson.M{"UserID": IntUserID}).Decode(&res)
		count, err := collection.CountDocuments(ctx, bson.M{"UserID": IntUserID})
		fmt.Println(count)
		//PageNation:=count/5;
		...
		if res.Email != "" || res.Name != "" || res.Password != "" {
			...
			FinalOptions := options.Find()
			FinalOptions.SetSkip((IntPageNeed - 1) * int64(perPage))
			FinalOptions.SetLimit(int64(perPage))
			cursor, _ := PostCol.Find(ctx, bson.M{"Email": res.Email}, FinalOptions)
